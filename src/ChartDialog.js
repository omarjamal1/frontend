	...
	handleChange (event) {
		...
		if (event.target.name === 'sensors'){
			if (event.target.value.length > 0){
				fetch(`http://dev.agviewer.net/extracts/${event.target.value[0].id}/`)
...
class ModalForm extends Component {
	...
	addGraph () {
		let url = `http://dev.agviewer.net/graphs/${this.props.chartId}/`,
		options = {
			method:'POST',
	...
	removeGraph (graph) {
		let url = `http://dev.agviewer.net/graph/${graph}/`,
		options = {
			method:'DELETE',
	        headers: {
	            "Content-Type": "application/json; charset=utf-8",
